...
stablem <- function(corp, lm, trnch="", mod="R") {
...
coocA <- function(corp, piv, poscooc="QLF SUB VBE", dis=5, coeff="dice", selec=1, nbs=30, D=0, F=Inf, attr="", val="") {
  ...
  if (attr=="" & val=="") { #KN : Wyszukuje pivot jako lemma i zwraca dump identyfikatorów
  ...
  if (attr!="" & val!="") {
    ...
    dump.pivot <- cqi_dump_subcorpus(sscorp2)[,1] #Pozycje znalezionego lemma
  ...
  pos.piv <- cqi_cpos2str(corp.ps ,cpos.pivot[1]) #KN: zwraca string znalezionych pozycji
  ...
  lst.pos.tt <- cqp_flist(CORP, "pos") # liste complète des POS du corpus wraz z frekwencją
  ...
  for (i in 1:nbpos.c) {  # on traite "radical" par "radical" KN:dla wybranej liczby POS
  ...
  n <- 0
  for (i in dis:1) {  # à gauche, traitement par colonne #KN: pobiera na podst. cpos wyrazy kookurująca
    n <- n + 1
    #KN: wędruje przez i, tj od 1 aż do końca okna
    cpos.G <- cpos.pivot - i #KN: oblicza pozycję kolokatu
    ids.G <- cqi_cpos2id(corp.lm, cpos.G)   #KN: zamień pozycję kolokatów na id
    ids.tablex[,n] <- ids.G #KN: zapisuje całą kolumnę pozycji
    ps.G <- cqi_cpos2str(corp.ps, cpos.G)  #KN: zapisuje całą kolumnę wg .pos
  ...
  for (i in 1:dis) {  # à droite, traitement par colonne
    ...
    ids.GD <- c(ids.GD, ids.D) #Łączy obie części w jeden wektor
    ps.GD <- c(ps.GD, ps.D)
    cat("***-")
  }
  ...
  #KN: wybiera tylko >3 wystąpienia
  ...
  #
  #KN: Oblicza frekwencję danego kolokatu w korpusie
  ...
  eff.piv <- rep(eff.pivot, nrow(cooc.final)) #Powtarza ilość lemmatu w korpusie x ilość kolokatów
  cooc.final[,6] <- as.numeric(eff.piv)
  E11 <- as.numeric(cooc.final[,5])*as.numeric(cooc.final[,6])/as.numeric(eff.tt.final)#KN: expected E11
  cooc.final[,7] <- E11
  ...
  resultat <- cooc.final[rev(order(cooc.final[,numcol])),] #Lista uporządkowana wg współczynnika, tu: Dice
  nb.lignes <- min(nbs, nrow(resultat))#Obcina listę do nbs=30 lub mniej (jeśli znaleziono mniej kolokatów)
  resultat <- resultat[1:nb.lignes,]
  ord.res <- order(resultat[,2], -resultat[,numcol])
  resultat <- resultat[ord.res,] #Tabela zawierająca wszystkie współczynniki
  #KN: tabela tylko DIce
  cooc.affich.p <- as.data.frame(cbind(resultat[,2],resultat[,4],resultat[,3],resultat[,5],resultat[,numcol]))
  cooc.affich <- cooc.affich.p[1,] #Pierwszy wiersz
  nm.affich <- c("POS", "lemme", "eff.cooc", "eff.lemme", coeff)#Nazwy kolumn
  cooc.affich <- rbind(nm.affich,rep(" ", 5), cooc.affich)#Matryca z jednym pustym rzędem (dla estetycznego wyświetlania?)
  
  for (i in 2:nrow(cooc.affich.p)) { #z początkowej pełnej listy wiersze 2-końca
    if (cooc.affich.p[i,1]==cooc.affich.p[i-1,1]) cooc.affich <- rbind(cooc.affich, cooc.affich.p[i,])#KNL ??? sprawdza dublety???
    if (cooc.affich.p[i,1]!=cooc.affich.p[i-1,1]) cooc.affich <- rbind(cooc.affich, rep(" ",5))  
  }
  write.matrix(cooc.affich,file="") #KN: Estetycznie wypisuje matrycę
  ...
  #KN: cooc0L 
  cooc0 <- data.frame(id=cooc.final[,1],pos=cooc.final[,2],cooc=cooc.final[,3], lem=cooc.final[,4],eff.lem=cooc.final[,5],eff.pivot=cooc.final[,6],Dice=cooc.final[,8],PoissonS=cooc.final[,9],Daille=cooc.final[,10],HyperG=cooc.final[,12],PMI=cooc.final[,11],cum="")#Lista uporządkowana wg lemma
  cooc1 <- data.frame(id=resultat[,1],pos=resultat[,2],cooc=resultat[,3], lem=resultat[,4],eff.lem=resultat[,5],eff.pivot=resultat[,6],Dice=resultat[,8],PoissonS=resultat[,9],Daille=resultat[,10],HyperG=resultat[,12],PMI=resultat[,11],cum="")#Lista uporządkowana wg Dice'a
  DF <- paste(D,F, sep="//")#tlex=ids.tablex = lista pozycji kolokatów
...
coocB <- function(tc, cex=.7, marg=4, prox="dice", selec=1){
  ...
  # cas simple
  if (attr=="" & val=="") {
    for (i in 1:(larg-1)) {#KN: Każdą z kolokacji ...
      prem <- i + 1
      cat("*") #KN: () wyświetl gwiazdkę )
      for (j in prem:larg) { #KN: ... porównaj ze wszystkimi następnymi i
        n <- n + 1
        a <- lico[i] #KN: Dobiera w pary ciągi i + kolejna kolokacja j
        b <- lico[j]
        #KN: Wyszukuje oba wyrazy w sąsiedztwie [ a {}4 b i b {}4 a ] max. marg (dom. 4) innych
        dem1 <- paste("[lemma = '" , a , "'][]{0," , marg , "}[lemma = '" , b , "'] within s", sep="")
        dem2 <- paste("[lemma = '" , b , "'][]{0," , marg , "}[lemma = '" , a , "'] within s", sep="")
        N1 <- subcorpus(c, dem1) #KN: Tworzy subcorpusy z obu wyszukiwań
        N2 <- subcorpus(c, dem2)
        eff <- size(N1) + size(N2) #KN: Ile jest wspólnych wystąpień obu kolokacji?
        vdis3[n] <- eff #KN: Zapisz ilość wspólnych wystąpień obu kolokatów
        vdis1[n] <- lico[i] #KN: Zapisz node
        vdis2[n] <- lico[j] #KN: Zapisz kolokat
        vdis4[n] <- fr[i] #KN: Zapisz ilość wystąpień 1 wyrazu w korpusie
        vdis5[n] <- fr[j] #KN: Zapisz ilość wystąpień 2 wyrazu w korpusie
      }
    }
  }
  ...
  vdis6 <- vdis3 * 1000 / (vdis4 + vdis5)^selec #KN: Dice: ilość wspólnych wystąpień par kolokacji / ilość pojedyńczych
  vdis6 <- sprintf("%06.3f", vdis6)
  vdis <- cbind(vdis1, vdis2, vdis3, vdis4, vdis5, vdis6) #KN: tworzy df ze współczynników
  vdis <- as.data.frame(vdis, stringsAsFactors=FALSE)
  names(vdis) <- c("lm1", "lm2", "cooc", "ef_lm1", "ef_lm2", "Dice")   #Tabela ma teraz formę: vdis1 vdis2 vdis3 vdis4 vdis5 vdis6 //  "diluo" "incurro" "0" "214" "553" "00.000"
  
  # création de la matrice des distances
  cat("\n")
  dice <- vdis[,6]
  bid <- rep(0,larg^2) #KN: larg= ilość kolokatów (domyślnie 30)
  tabdisLm <- matrix(bid, nrow=larg, ncol=larg)#KN: Tworzy pustą tablicę wypełnioną bid=0
  n <- 0
  for (i in 1:(larg-1)) { #KN: Uzupełnij matrycę obliczonymi wcześniej danymi, tj. Duce
    prem <- i + 1
    for (j in prem:larg) {
      n <- n + 1
      tabdisLm[i,j] <- dice[n]
  ...
  rownames(tabdisLm) <- lico # KN: Nazwij kolumny i wiersze kolokacjami
  colnames(tabdisLm) <- lico
  
  vdis <- vdis[rev(order(vdis[,6])),] #Uporządkuj tabelę kolokacji wg malejącej wartości Dice
  df$coocT <- vdis
  df$distmatT <- tabdisLm
  tabdisLm <- log1p(tabdisLm ^ .2) #KN: Logarytmuj matrycę kolokacji
  ...
  idlem <- tc$cooc1[,c(1,4)] #KN: Z pierwotnej tablicy kolokatów (coocA) wybierz kolumnę lemma i id kolokatu
  idlem <- idlem[(order(idlem[,2])),] # KN: ... uporządkuj wg id lemmatów
  idutils <- idlem[,1] #KN: skopiuj id lemmatów
  tlexb <- tlex %in% idutils #KN: sprawdź, gdzie w cooc$tlex (tj. w 10 wyrazach w L i P kontekście node) znajdują się id 30 wybranych kolokacji
  tlexb <- matrix(tlexb, ncol=ncol(tlex), nrow=nrow(tlex))
  tlexS <- apply(tlexb, 1, sum) #KN: Ile wystąpień kolokatu znajduje się w jednym rzędzie
  tlexu <- tlex[(tlexS>1),] #KN: Skopiuj z tlex te wiersze id kontekstu, w których  znaleziono więcej niż jeden kolokat
  tlexcl <- alply(tlexu, 1, match, idutils)#KN: na której pozycji w wektorze idutils znajduje się kolokat o f>1 (czyli tlexu)
  tlexcl <- t(as.data.frame(tlexcl))
  
  cretab <- function(x) {
    lig0 <- rep(0,50)#KN: Tworzy wektor 50-u zer
    lig1 <- x[!is.na(x)]#KN: Znajduje numery kolokatów na liście 30u znalezionych wcześniej
    lig0[lig1] <- 1#KN: Wstawia 1 w tym miejscu wektoru 50-u zer, które koresponduje z numerem kolokatu
    return (lig0)
  }
  
  tabsup <- alply(tlexcl, 1, cretab) #KN: ???Tworzy listę: 1) Tworzy rząd 50-u zer (cretab), w których niepuste wartości zastąpione są 1
  tabsup <- t(as.data.frame(tabsup)) #KN: odwraca matrycę
  tabsup <- tabsup[,(1:nb)] #KN: przycina tablicę do ilości znalezionych kolokatów
  colnames(tabsup) <- idlem[,2]
  
  # analyse en composantes principales
  #  fréquences totales (dans le corpus)
  tab.pca <- dudi.pca(tabdisLm, scannf=FALSE, nf=4) #KN: Na matrycy kolokat:kolokat (dice) przeprowadź PCA ...
  ...
  n <- 0
  for (i in 1:(nb-1)) {#KN: Przerabia matrycę korelacji na df o postaci kol1:kol2:eff
    prem <- i + 1
    for (j in prem:nb) {
      n <- n + 1
  ...
  discor[,4] <- T0[R1r, 1]  #KN: uzupełnia df korelacji o T0
  discor[,5] <- T0[R1r, 2]
  discor[,6] <- T0[R2r, 1]
  discor[,7] <- T0[R2r, 2]
  discor1 <- discor[(1:12),]
  ...
  return(list(df,plot2=plot2a,plot1=plot1a ) )
}
